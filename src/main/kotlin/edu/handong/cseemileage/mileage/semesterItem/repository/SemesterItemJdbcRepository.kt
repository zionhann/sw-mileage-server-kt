...
class SemesterItemJdbcRepository(private val jdbcTemplate: JdbcTemplate) {
    ...
    fun insertSemesterList(semesterList: List<SemesterItem>) {
        jdbcTemplate.batchUpdate(
            "insert into _sw_mileage_semester_item (point_value, item_max_points, semester_name, item_id, category_id, category_max_points, mod_date, reg_date) values (?, ?, ?, ?, ?, ?, ?, ?)",
            object : BatchPreparedStatementSetter {
                override fun setValues(ps: PreparedStatement, i: Int) {
                    ...
                    ps.setString(6, semesterList[i].categoryMaxPoints.toString())
